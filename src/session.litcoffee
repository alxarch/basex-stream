...
All incoming socket data is piped to the [Clean](streams.html#Clean) filter
to handle 0x00 and 0xFF bytes and break incoming data into usable chunks.

				input = new Clean
				socket.pipe input

To handle server responses the 'clean' input stream is handed over to the [Parser](parser.html).
				
				{parse} = require "./parser"

				read = parse input

All outgoing socket data goes through a [Smudge](stream.html#Smudge) filter
prior to sending to handle 0x00 and 0xFF padding.

				{Smudge} = require "./stream"
				output = new Smudge
				output.pipe socket
...
it ecounters.
				...
				write = (data...) =>
					Promise.reduce data, (written, item) =>
						new Promise (resolve, reject) ->
							if item instanceof Readable
									...
									item.on "end", ->
										item.unpipe output
										resolve written.push yes
							else if item?
								output.write item, -> resolve written.push yes
							else
								resolve written.push no
...
only after the connection is established.
			...
			@io = (callback) -> io.spread -> callback...
...
Digest authentication is used in BaseX version >= `8.x`.
...
> If `data` argument is a `Readable` stream then it will be piped directly to the session output.
